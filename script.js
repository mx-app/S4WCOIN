...
// DOM Elements for Caesar Cipher Puzzle
...
function displayNextCaesarPuzzle() {
    // البحث عن اللغز التالي الذي لم يتم حله
...
async function handleCorrectCaesarSolution() {
    ...
    // التحقق من أن المستخدم لم يحل هذا اللغز من قبل
    if (!solvedCaesarPuzzles.has(CaesarPuzzleID)) {
        // إضافة اللغز إلى الألغاز التي تم حلها
...
async function updateUserCaesarPuzzleProgress(CaesarPuzzleID) {
    ...
    // تحديث تقدم الألغاز في قاعدة البيانات
    const { error } = await supabase
        .from('users')
        .update({
...
// التحقق من أن المستخدم لم يحصل على المكافأة مرتين
async function checkUserCaesarPuzzleProgress(CaesarPuzzleID) {
    const userTelegramId = uiElements.userTelegramIdDisplay.innerText;

    // جلب تقدم المستخدم من قاعدة البيانات
    const { data, error } = await supabase
        .from('users')
        .select('CaesarSolvedPuzzles')
        .eq('telegram_id', userTelegramId)
        .maybeSingle();
    ...
    const solvedPuzzlesData = data?.CaesarSolvedPuzzles || [];

    // التحقق مما إذا كان المستخدم قد حل هذا اللغز من قبل
    return solvedPuzzlesData.includes(CaesarPuzzleID);
}

// إضافة حدث لزر "CaesarPuzzleBtn" لعرض اللغز
CaesarPuzzleBtn.addEventListener('click', function() {
    CaesarPuzzleContainer.classList.remove('hidden');
    displayNextCaesarPuzzle();  // عرض اللغز عند الضغط على الزر
});
