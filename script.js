...
async function fetchPuzzles() {
    try {
        ...
        showNotification('Failed to load puzzles.');
        console.error(error);
    }
}
...
function loadNextPuzzle() {
    ...
    if (availablePuzzles.length > 0) {
        ...
        showNotification('No more puzzles available.');
    }
}
...
function handleCorrectSolution() {
    ...
    showNotification(`Correct! You've earned ${currentPuzzle.reward} coins.`);
    loadNextPuzzle();
}
...
function handleWrongSolution() {
    ...
    if (remainingAttempts > 0) {
        showNotification(`Wrong answer. You have ${remainingAttempts} attempts remaining.`);
    } else {
        showNotification('You have used all your attempts. Please try again tomorrow.');
        disablePuzzle();
    }
}
...
async function saveSolvedPuzzle() {
    ...
    if (error) {
        ...
        showNotification('Error fetching your progress.');
        return;
    }
    ...
    if (solvedCiphers.includes(currentPuzzle.CaesarPuzzleID)) {
        showNotification('You have already solved this puzzle and received the reward.');
        return;
    }
    ...
    if (updateError) {
        ...
        showNotification('Error saving your progress. Please try again.');
    } else {
        showNotification(`Progress saved. You've earned ${currentPuzzle.reward} coins.`);
...
function updateBalance(reward) {
    ...
    showNotification(`Balance updated: +${reward} coins.`);
}
...
async function loadUserProgress() {
    ...
    if (error) {
        ...
        showNotification('Error loading your progress.');
        return;
    }
