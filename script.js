...
function displayPuzzle() {
    ...
    remainingAttempts = maxAttempts; // إعادة تعيين المحاولات عند تحميل أحجية جديدة
}
...
function handleCorrectSolution() {
    ...
    setTimeout(loadNextPuzzle, 500); // إضافة مهلة قصيرة قبل عرض الأحجية التالية
}
...
async function saveSolvedPuzzle() {
    ...
    const { updateError } = await supabase
        .from('users')
        .update({ caesar_ciphers: solvedCiphers }) // تعديل المتغير ليكون مطابقًا
        .eq('telegram_id', userTelegramId);
